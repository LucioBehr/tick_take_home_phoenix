defmodule TickTakeHomeWeb.Wallet.Logic do
  ...
  alias TickTakeHome.Models.{Users, Balances, Assets, Wallets}
    ...
    case Repo.get(Assets, asset) do
      %Assets{} ->
        if balance do
          ...
          Repo.insert!(Balances.changeset(%Balances{}, %{"user_id" => user_id, "asset_id" => asset, "available" => amount, "frozen" => amount}))
        end
        {:ok, "Deposited #{amount} #{asset} for user #{user_id}"}
      _ -> {:error, "Error: Asset #{asset} does not exist"}
    end
  end
  ...
  def withdraw(%{"user_id" => user_id, "asset" => asset, "amount" => amount}) do
    with %Assets{} <- Repo.get(Assets, asset),
      ...
      if balance.available >= amount do
        Repo.one(from b in Balances, where: b.user_id == ^user_id and b.asset_id == ^asset)
        |> Repo.update(set: [available: balance.available - amount])
